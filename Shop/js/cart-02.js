...
window.addEventListener('click', function(event) {
  ...
  if (event.target.hasAttribute('data-cart')) {
    ...
    const card = event.target.closest('.card');
      ...
      // Собираем данные с этого товара и зыписываем их в единый объект productItems
    const productItems = {
      id: card.dataset.id,
    ...
    const itemInCart = cartWrapper.querySelector(`[data-id="${productItems.id}"]`);

      // Если товар есть в корзине
    if (itemInCart) {
      const counterElement = itemInCart.querySelector('[data-counter]');
      counterElement.innerText = parseInt(counterElement.innerText) + parseInt(productItems.counter);
